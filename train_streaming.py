...
def train(args):
    ...
    for rollout_id in range(args.start_rollout_id, args.num_rollout):
        ...
        logger.info(f"[DRIVER] Gradient sync + optimizer step took {time.time() - sync_start:.2f}s")
        ...
        logger.info("[DRIVER] Calling update_weights() (mode=training)...")
        elastic_group.update_weights()
        logger.info("[DRIVER] update_weights() done, calling switch_all_to_inference()...")
        elastic_group.switch_all_to_inference()
        logger.info("[DRIVER] switch_all_to_inference() done")
