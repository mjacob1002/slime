"""DEBUG: 2-rollout streaming + migration + tail-split combo.

Stack the tail-split heuristic on top of train_group_aware migration to
measure the combined wall-clock effect. Compare against:
  - the prior 10-rollout migration FIXED run (first 2 rollouts: 597s / 567s)
  - the streaming-only tail-split DEBUG (599s / 579s)
"""
import os
...
def execute():
    ...
    ci_args = (
        ...
        "--perfetto-trace-path /tmp/DEBUG_streaming_8gpu_tp_train2_tp_infer1_deepseek8b_2rollout_tail_split_migration_trace.json "
