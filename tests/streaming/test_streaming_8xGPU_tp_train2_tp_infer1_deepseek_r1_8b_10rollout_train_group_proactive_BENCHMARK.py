"""BENCHMARK: 10-rollout streaming + train_group_proactive migration + adaptive clear.

Direct A/B comparison to the train_group_aware 10-rollout benchmark, using the
same replay file so per-rollout slots are apples-to-apples.

Migration policy: train_group_proactive (combined Mode A inter-group +
Mode B intra-group with lone-group trigger and un-drain).
Grab policy: tail_split.
Eager-sleep gate ON (driven by --migration-policy=train_group_proactive).
Adaptive clear_memory via SLIME_CLEAR_MEM_RESERVED_GB=110.

Reference baselines (same replay file workload):
  - colocate 10r:                       6619s
  - streaming + train_group_aware 10r:  5922s (just completed)
  - streaming + train_group_proactive:  THIS RUN
...
def execute():
    ...
    ci_args = (
        ...
        "--perfetto-trace-path /tmp/BENCHMARK_streaming_8gpu_tp_train2_tp_infer1_deepseek8b_10rollout_train_group_proactive_trace.json "
