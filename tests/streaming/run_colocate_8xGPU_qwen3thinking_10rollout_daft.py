"""10-rollout 8xGPU colocate Qwen3-30B-A3B-Thinking on daft-math (FrontierMath proxy).

daft-math has 199 prompts → rollout_batch_size=199 (entire dataset per rollout).
"""
import os
...
DATASET_NAME = "daft-math"
RUN_TAG = "qwen3thinking_daft"
...
def execute():
    ...
    rollout_args = (
        ...
        "--reward-key score "
        "--num-rollout 10 "
        "--rollout-batch-size 199 "
        "--n-samples-per-prompt 4 "
        "--rollout-max-response-len 65536 "
        "--rollout-temperature 1 "
        "--global-batch-size 796 "
    )
