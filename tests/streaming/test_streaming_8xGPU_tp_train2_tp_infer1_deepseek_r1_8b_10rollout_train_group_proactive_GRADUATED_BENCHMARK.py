"""BENCHMARK: 10-rollout streaming + train_group_proactive + GRADUATED tail-split.

Tests whether stepping the per-grab cap down 8 -> 4 -> 2 -> 1 (as
remaining_to_train falls through 32 -> 16 -> 8) gives additional wall-clock
savings on top of the proactive intra-group migration.

Layered comparison (all 10-rollout, same DAPO-math replay file):
  - streaming + train_group_aware     + tail_split:           5922s
  - streaming + train_group_proactive + tail_split:           5843s
  - streaming + train_group_proactive + graduated_tail_split: THIS RUN
...
def execute():
    ...
    ci_args = (
        ...
        "--perfetto-trace-path /tmp/BENCHMARK_streaming_8gpu_tp_train2_tp_infer1_deepseek8b_10rollout_train_group_proactive_GRADUATED_trace.json "
