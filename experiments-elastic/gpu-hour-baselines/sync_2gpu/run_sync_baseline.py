...
def run_sync_experiment(
    ...
    # GPU configuration - 2 GPUs colocated (both do inference together, then training together)
    ...
    """
    Run a synchronous train.py experiment with 2 GPUs in colocated mode.
    Both GPUs do inference together, then switch to training together.
    """
    ...
    # Execute using synchronous train.py instead of train_async.py
...
if __name__ == "__main__":
    ...
    output = run_sync_experiment(capture_output=True)
    if output:
        print(output)
    else:
